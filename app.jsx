...
const APP_LAST_UPDATED = "2026/08/26 17:06";
    ...
    <div style={{ background: COLORS.paper, border: `1.5px solid ${COLORS.line}`, borderRadius: 10, padding: 20 }}>
      ...
      <GuideSection title="③ アルバイト管理">
        <GuideItem label="勤怠入力">
          従業員・日付・開始/終了時刻(15分単位)を入力して記録します。「ランク」で「通常/呼び込み(+1)/同伴(+2)」を選ぶと、その勤務日の時給がアルバイトマスタで設定したランク別時給に切り替わります(複数の条件に該当する場合も、最も高いランクのみ適用されます)。日給を入力した場合は、時給×時間の計算より日給が優先されます(ランクは無視されます)。同じ従業員・同じ日付ですでに登録されている時間帯と重なる場合は保存できません(エラーメッセージが表示されます)。
        </GuideItem>
        ...
        <GuideItem label="集計">
          月次・年次で従業員ごとの給与を自動集計します(ランクによる時給アップ分も反映されます)。
        </GuideItem>
        <GuideItem label="アルバイトマスタ">
          従業員の氏名・時給を登録・編集・削除します。あわせて、「呼び込み」「同伴」のランクに応じた時給も個別に設定できます(未設定の場合は通常の時給が使われます)。
        </GuideItem>
      </GuideSection>
