...
const APP_LAST_UPDATED = "2026/08/26 17:38";
...
function defaultData() {
  return {
    ...
    serviceChargeRate: 0, // %
    taxRate: 10, // %
    payroll: { employees: [], shifts: [], rankBonusRates: { call: 100, companion: 200, other: 0 } },
    ...
    <div style={{ background: COLORS.paper, border: `1.5px solid ${COLORS.line}`, borderRadius: 10, padding: 20 }}>
      ...
      <GuideSection title="③ アルバイト管理">
        <GuideItem label="勤怠入力">
          従業員・日付・開始/終了時刻(15分単位)を入力して記録します。「ランク」で「呼込み/同伴/その他」のいずれかを選ぶと、アルバイトマスタで設定した金額がその勤務日の時給に1時間あたり加算されます(選び直す・もう一度押して解除することもできます。複数は同時に選べません)。「同伴バック」「売上バック」は勤務時間に関係なく加算される金額です。日給を入力した場合は、時給×時間の計算より日給が優先されます(ランク・バックはそのまま加算されます)。同じ従業員・同じ日付ですでに登録されている時間帯と重なる場合は保存できません(エラーメッセージが表示されます)。
        </GuideItem>
        <GuideItem label="勤怠一覧">
          「個別」で従業員ごと、「全員一括」で全員分の勤怠記録を確認できます。「本日のみ」「すべて」、または日付を指定して絞り込めます。一覧上部に該当件数・合計金額を表示します。「編集」ボタンから、勤怠入力時に選んだランクを含めて内容を変更できます。「CSVダウンロード」で表示中のデータを書き出せます。
        </GuideItem>
        <GuideItem label="集計">
          月次・年次で従業員ごとの給与を自動集計します(ランクによる時給アップ分・バックも反映されます)。
        </GuideItem>
        <GuideItem label="アルバイトマスタ">
          左側で従業員の氏名・時給を登録・編集・削除します。右側の「ランク別時給アップ額」では、勤怠入力の「呼込み/同伴/その他」を選んだ際に時給へ加算する金額(1時間あたり)を、全従業員共通で設定できます。
        </GuideItem>
      </GuideSection>
